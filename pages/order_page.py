...
class OrderPage(BasePage):
    ...
    def create_order_and_get_number(self):
        self.drag_and_drop_element(OrderPageLocators.FIRST_INGREDIENT_IN_LIST,
                                   MainPageLocators.INGREDIENTS_IN_BURGER_LIST)
        self.click_on_element(MainPageLocators.CONFIRM_ORDER_BUTTON)
        WebDriverWait(self.driver, 999).until_not(expected_conditions.text_to_be_present_in_element(
            OrderPageLocators.CREATED_ORDER_FORM, '9999'))
        order_number = self.get_text_from_element(OrderPageLocators.CREATED_ORDER_FORM)
        self.click_on_element(OrderPageLocators.CREATED_ORDER_FORM_CLOSE)
        return order_number
    ...
    def show_all_time_done_orders_before_new(self):
        ...
        self.click_on_element(MainPageLocators.LOGO_BUTTON)
